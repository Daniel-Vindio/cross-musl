...
cat > /cross-tools/bin/lex << "EOF"
#!/bin/bash
# Begin /cross-tools/bin/lex

exec /cross-tools/bin/flex -l "$@"

# End /cross-tools/bin/lex
EOF
registro_error "cat > /cross-tools/bin/lex"

chmod -v 755 /cross-tools/bin/lex
registro_error "chmod -v 755 /cross-tools/bin/lex"
