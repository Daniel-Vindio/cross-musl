...
#Al ser una instación normalizada no hace falta:
#Symlint de libc.so --> ld-musl-${CLFS_ARCH_m}.so.1
#Tampoco hace falta el archivo:
#etc/ld-musl-${CLFS_ARCH_m}.path
#ya que las bibliotecas están en rutas normalizadas.
#/lib:/usr/lib etc.
